...
Currently (as 2022 Q3), \code{stcp} package does not directly support this opposite directional test.
However, we can simply define a new random variable $Y := 1-X$ with $\theta := \mathbb{E}Y = 1-\mu$ and test the corresponding hypotheses.
...
In this case, we need to [TODO:ADD explanation about combining two objects]

```{r, two_side_null}
test_run4 <- combine_stcp_run(test_run2, test_run3)
...
m_post <- 2 / (2+7)
plot(1:max_sample, x_vec, pch=20, 
     xlab = "n", ylab = expression('X'['n']), main = "Simulated Data")
```
